...
# https://github.com/AliyunContainerService/kubernetes-issues-solution/blob/master/kubelet/kubelet.sh
...
fix_orphanedPod(){
    ...
    if [ -d /var/lib/kubelet/pods/$podid/volume-subpaths/ ]; then
        ...
        rm -rf /var/lib/kubelet/pods/$podid/volume-subpaths/
    fi
...
do
    ...
    do
        ...
        elif [[ $item == *"Orphaned pod"* ]] && [[ $item == *"but volume subpaths are still present on disk"* ]]; then
            fix_orphanedPod $item            
