...
fileHandler = handlers.RotatingFileHandler('clean.log', 'a', 10 * 1024 * 1024, 2)
...
if __name__ == "__main__":
    ...
    registry.auth_schemes = get_auth_schemes(registry, '/v2/_catalog')
    image_list = registry.list_images()
    # log.info(pprint.pformat(image_list))
    # tag_dict = {}
    #
    # for image_name in image_list:
    #     tag_dict.setdefault(image_name, registry.list_tags(image_name))
    # log.info(pprint.pformat(tag_dict))
    image_name = image_list[-1]
    taglist = registry.list_tags(image_name)
    ...
    def natural_keys(text):
        ...
        return [__atoi(c) for c in re.split('(\d+)', text)][1]


    taglist = sorted(taglist, key=natural_keys)
    # log.info(pprint.pformat(image_config))
    # image_age = registry.get_image_age(image_name, image_config)
    # log.info(image_age)
    log.info(pprint.pformat(taglist))
